...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = {
      ...
      total_credits_used: logs?.reduce((sum: number, log: any) => sum + (log.credits_used || 0), 0) || 0,
      total_names_generated: logs?.reduce((sum: number, log: any) => sum + (log.names_generated || 0), 0) || 0,
      avg_per_session: logs?.length > 0
        ? (logs.reduce((sum: number, log: any) => sum + (log.names_generated || 0), 0) / logs.length)
        : 0
    };
